...
const WARN = (...args: any[]) => console.warn('[KefinTweaks RemoveContinue]', ...args);
const ERROR = (...args: any[]) => console.error('[KefinTweaks RemoveContinue]', ...args);

const RemoveContinue: React.FC = () => {
    // Function to fetch current user data for an item
    const fetchUserData = async (itemId: string) => {
        // @ts-ignore
    ...
    const updateUserDataProgress = async (itemId: string, userData: any) => {
        // @ts-ignore
        const ApiClient = window.ApiClient;
        const userId = ApiClient.getCurrentUserId();
        const serverUrl = ApiClient.serverAddress();
    ...
    const addRemoveContinueButton = useCallback((overlayContainer: Element) => {
    ...
    useEffect(() => {
        ...
        const processExistingOverlayContainers = () => {
                ...
                if (mutation.type === 'childList') {
                    ...
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            if (elementNode.classList && elementNode.classList.contains('cardOverlayContainer')) {
                                const buttonContainer = elementNode.querySelector('.cardOverlayButton-br');
                                if (buttonContainer) {
                                    addRemoveContinueButton(elementNode);
                                }
                            }

                            // Check for overlay containers within the added node
                            const overlayContainers = elementNode.querySelectorAll && elementNode.querySelectorAll('.cardOverlayContainer');
    ...
    }, [addRemoveContinueButton]);

    return null;
};

export default RemoveContinue;