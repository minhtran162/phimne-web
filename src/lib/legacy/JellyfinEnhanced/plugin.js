...
(function() {
    ...
    const RE_ICON_TOKEN = /\{\{icon:([a-zA-Z]+)\}\}/g;
    ...
    window.JellyfinEnhanced = {
        ...
        escapeHtml: (str) => {
            ...
            return str.replace(RE_HTML_ESCAPE, (m) => HTML_ESCAPES[m]);
        },
            ...
            if (params && typeof params === 'object') {
                for (const param in params) {
                    if (Object.prototype.hasOwnProperty.call(params, param)) {
                        // Use split/join for better performance than new RegExp in a loop for simple string replacement
                        text = text.split(`{${param}}`).join(params[param]);
                    }
                }
            }

            // Replace {{icon:name}} tokens with emoji icons
            text = text.replace(RE_ICON_TOKEN, (match, iconName) => {
                const iconKey = iconName.replace(RE_CAMEL_CASE, '$1_$2').toUpperCase();
                return EMOJI_ICONS[iconKey] || match;
            });
        ...
        t: (key, params = {}) => { // Actual implementation defined later
            ...
            text = text.replace(/\{\{icon:([a-zA-Z]+)\}\}/g, (match, iconName) => {
                ...
                return emojiIcons[iconKey] || match;
            });
        ...
        try {
            ...
            results.forEach(result => {
                if (result.status === 'fulfilled' && result.value) {
                    ...
                    } else if (data.name === 'shortcuts') { JE.userConfig.shortcuts = { Shortcuts: [] }; } else if (data.name === 'bookmark') { JE.userConfig.bookmark = { bookmarks: {} }; } else if (data.name === 'elsewhere') { JE.userConfig.elsewhere = {}; } else if (data.name === 'hiddenContent') { JE.userConfig.hiddenContent = { items: {}, settings: {} }; } else { JE.userConfig[data.name] = {}; }
                } else {
