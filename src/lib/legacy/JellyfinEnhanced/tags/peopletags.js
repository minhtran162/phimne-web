...
(function(JE) {
    ...
    JE.initializePeopleTags = function() {
        ...
        // Precompute lowercased country map for O(1) lookup
        const LOWERCASE_COUNTRY_MAP = {};
        for (const [countryName, code] of Object.entries(COUNTRY_MAP)) {
            LOWERCASE_COUNTRY_MAP[countryName.toLowerCase()] = code;
        }

        const peopleCache = JSON.parse(localStorage.getItem(CACHE_KEY) || '{}');
        const peopleCacheTimestamp = JSON.parse(localStorage.getItem(CACHE_TIMESTAMP_KEY) || '{}');
        ...
        function getCountryCodeFromBirthPlace(placeString) {
            ...
            return LOWERCASE_COUNTRY_MAP[lastPart] || null;
        }
        ...
        function initialize() {
            ...
            JE.helpers.createObserver(
                ...
                (mutations) => {
                    ...
                    // Check if any mutations are relevant to cast sections
