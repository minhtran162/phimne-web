...
(function (JE) {
    ...
    JE.initializeQualityTags = function() {
        ...
        const RE_IMAX = /\bIMAX(?:[ ._-]?ENHANCED)?\b/i;
        const RE_NON_IMAX = /\bNON[ ._-]?IMAX\b/i;
        const RE_RESOLUTION = /\b(4k|2160p|1440p|1080p|720p|480p|360p|404p|384p|520p)\b/i;
        const RE_DOLBY_VISION = /dolby\s*vision|dv/i;
        const RE_HDR10_PLUS = /hdr10plus/i;
        const RE_HDR10 = /hdr10/i;
        const RE_HDR = /\bhdr\b/i;
        const RE_ATMOS = /atmos/i;
        const RE_TRUEHD = /truehd/i;
        const RE_DTSX = /dts-x/i;
        const RE_DTS = /\bdts\b/i;
        const RE_DDP = /dolby\s*digital\+/i;
        const RE_3D_FORMAT = /hsbs|fsbs|htab|ftab|mvc/;
        const RE_CHANNEL_71 = /\b7[. ]?1\b/;
        const RE_CHANNEL_51 = /\b5[. ]?1\b/;
        const RE_CHANNEL_20 = /\bstereo\b|\b2[. ]?0\b/;
        ...
        function getChannelTag(audioStreams) {
            ...
            for (const stream of audioStreams) {
                ...
                if (!detectedLayoutTag) {
                    if (RE_CHANNEL_71.test(layoutSignals)) {
                        detectedLayoutTag = '7.1';
                    } else if (RE_CHANNEL_51.test(layoutSignals)) {
                        detectedLayoutTag = '5.1';
                    } else if (RE_CHANNEL_20.test(layoutSignals)) {
        ...
        function getEnhancedQuality(mediaStreams, mediaSources, itemData = null) {
            ...
            if (imaxContext && RE_IMAX.test(imaxContext) && !RE_NON_IMAX.test(imaxContext)) {
                qualities.add('IMAX');
            }
            ...
            if (primaryVideoStream) {
                ...
                const resolutionMatch = displayTitle.match(RE_RESOLUTION);
            ...
            if (primaryVideoStream) {
                ...
                const dolbyVisionMatchTitle = displayTitle.match(RE_DOLBY_VISION);
                const dolbyVisionMatchRange = videoRangeType.match(RE_DOLBY_VISION);
                if (dolbyVisionMatchTitle || dolbyVisionMatchRange) {
                    hdrTag = 'Dolby Vision';
                    qualities.add(hdrTag);
                } else {
                    // Priority 2: HDR Fallback
                    const hdr10PlusMatchTitle = displayTitle.match(RE_HDR10_PLUS);
                    const hdr10PlusMatchRange = videoRangeType.match(RE_HDR10_PLUS);

                    if (hdr10PlusMatchTitle || hdr10PlusMatchRange) {
                        hdrTag = 'HDR10+';
                        qualities.add(hdrTag);
                    } else {
                        const hdr10MatchTitle = displayTitle.match(RE_HDR10);
                        const hdr10MatchRange = videoRangeType.match(RE_HDR10);

                        if (hdr10MatchTitle || hdr10MatchRange) {
                            hdrTag = 'HDR10';
                            qualities.add(hdrTag);
                        } else {
                            const hdrMatchTitle = displayTitle.match(RE_HDR);
                            const hdrMatchRange = videoRangeType.match(RE_HDR);
            ...
            for (let i = 0; i < audioStreams.length; i++) {
                ...
                const atmosMatch = displayTitle.match(RE_ATMOS);
                const truehdMatch = displayTitle.match(RE_TRUEHD);
                const dtsxMatch = displayTitle.match(RE_DTSX);
                const dtsMatch = displayTitle.match(RE_DTS);
                const ddpMatch = displayTitle.match(RE_DDP);
            ...
            if (mediaSources) {
                for (const source of mediaSources) {
                    if (source.Path) {
                        ...
                        const has3DFormat = RE_3D_FORMAT.test(path);
    ...
    JE.reinitializeQualityTags = function() {
        ...
        // Call the main initialize function to re-setup
        JE.initializeQualityTags();
        console.log(`${logPrefix} Feature re-initialized.`);
    };
