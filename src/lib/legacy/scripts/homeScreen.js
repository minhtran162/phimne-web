...
(function() {
        ...
        if (old.enableNewAndTrending !== undefined || old.enableNewMovies !== undefined) {
            ...
            if (old.customSections && Array.isArray(old.customSections)) {
                migrated.customSections = old.customSections.map(section => {
                    ...
                    const type = section.type || 'Collection';
                    const source = section.source || sourceId;
    ...
    async function getCollectionsData() {
        ...
        collectionsData = await window.ApiClient.getItems(window.ApiClient.getCurrentUserId(), {
    ...
    const renderedSections = new Set(); // Track rendered sections to avoid duplicates
    const isGeneratingDiscoverySections = false; // Prevent parallel generation (legacy)
    ...
    const preloadedSections = []; // Preloaded sections ready for instant rendering

    // Track rendered items to avoid duplicates
    const renderedActors = new Set(); // Track rendered actor IDs
    const renderedDirectors = new Set(); // Track rendered director IDs
    const renderedWriters = new Set(); // Track rendered writer IDs
    const renderedStudios = new Set(); // Track rendered studio IDs
    const renderedWatchedMovies = new Set(); // Track rendered watched movie IDs for "Because you watched" sections
    const renderedGenres = new Set(); // Track rendered genre IDs for spotlight sections
    const renderedNetworks = new Set(); // Track rendered network IDs for spotlight sections
    const renderedCollections = new Set(); // Track rendered collection IDs
    const renderedCustomDiscoverySections = new Set(); // Track custom sections used in discovery
    ...
    const renderedFavoriteMovies = new Set(); // Track rendered favorite movie IDs
    const renderedStarringWatchedMovies = new Set(); // Track movies used for "Starring X since you watched Y" sections
    const renderedDirectedWatchedMovies = new Set(); // Track movies used for "Directed by X since you watched Y" sections
    const renderedWrittenWatchedMovies = new Set(); // Track movies used for "Written by X since you watched Y" sections

    // Pre-rendered sections waiting to be revealed
    const hiddenDiscoverySections = []; // Array of pre-rendered section DOM elements

    // Discovery buffer system
    const discoveryBuffer = []; // Buffer holding groups of discovery items (always 2 groups)
    let additionalDiscoveryContent = true; // Flag indicating if more discovery content is available
    const preloadedSectionElements = []; // Preloaded section DOM elements (hidden)
    ...
    async function renderRecentlyAddedInLibrarySection(libraryId, sectionConfig, container) {
        ...
        let viewMoreUrl = `#/list.html?parentId=${libraryId}&serverId=${window.ApiClient.serverId()}`; // Default fallback
        ...
        const url = `${window.ApiClient.serverAddress()}/Users/${userId}/Items/Latest?${queryParams}`;
    ...
    async function getWatchlistUrl() {
        ...
        if (window.ApiClient._serverInfo.Version?.split('.')[1] > 10) {
            homePageSuffix = '';
        }
    ...
    async function renderImdbTop250Section(container) {
        try {
            ...
            const allMoviesResponse = await window.ApiClient.getItems(userId, options);
    ...
    async function fetchItemsByIds(itemIds) {
        try {
            ...
            const url = `${window.ApiClient.serverAddress()}/Items?Ids=${idsParam}&UserId=${userId}&EnableTotalRecordCount=false&EnableImageTypes=Primary,Backdrop,Thumb,Logo`;

            const response = await window.ApiClient.fetch({ url, method: 'GET' });
            const data = await response.json();
            ...
            const preloadPromises = [
                        ...
                        if (randomDirector && randomDirector.items && randomDirector.items.length > 0) {
                            ...
                            if (shuffledItems && shuffledItems.length > 0) {
                                return {
                                    ...
                                    data: randomDirector,
                                    items: shuffledItems,
                                    viewMoreUrl: `#/details?id=${randomDirector.id}&serverId=${window.ApiClient.serverId()}`
                                };
                            }
                        }
                    }
                        ...
                        if (randomWriter && randomWriter.items && randomWriter.items.length > 0) {
                            ...
                            if (shuffledItems && shuffledItems.length > 0) {
                                return {
                                    ...
                                    data: randomWriter,
                                    items: shuffledItems,
                                    viewMoreUrl: `#/details?id=${randomWriter.id}&serverId=${window.ApiClient.serverId()}`
                                };
                            }
                        }
                    }
                        ...
                        if (randomActor && randomActor.items && randomActor.items.length > 0) {
                            ...
                            if (shuffledItems && shuffledItems.length > 0) {
                                return {
                                    ...
                                    data: randomActor,
                                    items: shuffledItems,
                                    viewMoreUrl: `#/details?id=${randomActor.id}&serverId=${window.ApiClient.serverId()}`
                                };
                            }
                        }
                    }
                ...
                (async () => {
                    ...
                    if (randomNetwork) {
                        ...
                        const viewMoreUrl = `#/list.html?studioId=${randomNetwork.Id}&serverId=${window.ApiClient.serverId()}`;
                    ...
                    try {
                        ...
                        if (filteredItems.length > 0) {
                            ...
                            return {
                                ...
                                items: orderedItems,
                                viewMoreUrl: `#/details?id=${selectedActor.Id}&serverId=${window.ApiClient.serverId()}`
                            };
                        }
                    } catch (err) {
                    ...
                    try {
                        ...
                        if (filteredItems.length > 0) {
                            ...
                            return {
                                ...
                                items: orderedItems,
                                viewMoreUrl: `#/details?id=${selectedDirector.Id}&serverId=${window.ApiClient.serverId()}`
                            };
                        }
                    } catch (err) {
                    ...
                    try {
                        ...
                        if (filteredItems.length > 0) {
                            ...
                            return {
                                ...
                                items: orderedItems,
                                viewMoreUrl: `#/details?id=${selectedWriter.Id}&serverId=${window.ApiClient.serverId()}`
                            };
                        }
                    } catch (err) {
                ...
                (async () => {
                    ...
                    try {
                        ...
                        const moviesResponse = await window.ApiClient.getItems(window.ApiClient.getCurrentUserId(), {
                        ...
                        return {
                            ...
                            data: selectedGenre,
                            items: movies,
                            viewMoreUrl: `${window.ApiClient.serverAddress()}/web/#/list.html?genreId=${selectedGenre.Id}&serverId=${window.ApiClient.serverId()}`
                        };
                    } catch (err) {
                ...
                (async () => {
                    ...
                    try {
                        ...
                        const itemLimit = sectionConfig.itemLimit ?? (discoveryConfig.defaultItemLimit ?? defaultItemLimit);
                        const showsResponse = await window.ApiClient.getItems(window.ApiClient.getCurrentUserId(), {
                        ...
                        return {
                            ...
                            items: shows,
                            viewMoreUrl: `${window.ApiClient.serverAddress()}/web/#/list.html?studioId=${selectedNetwork.Id}&serverId=${window.ApiClient.serverId()}`
                        };
                    } catch (err) {
                ...
                (async () => {
                    ...
                    const availableSections = eligibleCustomSections.filter(section => section.id && !renderedCustomDiscoverySections.has(section.id));
    ...
    async function renderDiscoverySection(sectionData, container) {
        try {
            ...
            let sectionId = '';
            const order = DISCOVERY_ORDER + renderedSections.size;
    ...
    async function renderSection(sectionConfig, container, sectionIdPrefix) {
        try {
            ...
            if (source && source !== '') {
                switch (type) {
                    case 'Genre':
                        viewMoreUrl = `#/list.html?genreId=${source}&serverId=${window.ApiClient.serverId()}`;
                        break;
                    case 'Studio':
                        viewMoreUrl = `#/list.html?studioId=${source}&serverId=${window.ApiClient.serverId()}`;
                        break;
                    case 'Playlist':
                    case 'BoxSet':
                        viewMoreUrl = `#/details?id=${source}&serverId=${window.ApiClient.serverId()}`;
                        break;
                    case 'Parent':
                        viewMoreUrl = `#/list.html?parentId=${source}&serverId=${window.ApiClient.serverId()}`;
                        break;
                    case 'Tag':
                        viewMoreUrl = `#/list.html?type=tag&tag=${source}&serverId=${window.ApiClient.serverId()}`;
                        break;
                    default:
                        viewMoreUrl = null;
                        break;
