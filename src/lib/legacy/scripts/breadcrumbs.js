...
(function() {
    ...
    async function getTargetBreadcrumbStructure(item) {
        ...
        if (item.Type === 'Movie') {
            ...
            structure.elements = [
                { text: 'TVShows', url: `#/tv${urlSuffix}${queryParams}&tab=0`, clickable: true },
                { text: seriesName, url: `${protocol}//${host}/${protocol === 'https:' ? 'web/' : ''}#/details?id=${item.ParentId}&serverId=${ApiClient.serverId()}`, clickable: true },
            ...
            structure.elements = [
                { text: 'TVShows', url: `#/tv${urlSuffix}${queryParams}&tab=0`, clickable: true },
                { text: seriesName, url: `${protocol}//${host}/${protocol === 'https:' ? 'web/' : ''}#/details?id=${item.SeriesId}&serverId=${ApiClient.serverId()}`, clickable: true },
            ...
            structure.elements = [
                { text: 'Music', url: `#/music${urlSuffix}${queryParams}&tab=0`, clickable: true },
                { text: item.AlbumArtist, url: `${protocol}//${host}/${protocol === 'https:' ? 'web/' : ''}#/details?id=${item.ParentId}&serverId=${ApiClient.serverId()}`, clickable: true },
            ...
            structure.elements = [
                { text: 'Music', url: `#/music${urlSuffix}${queryParams}&tab=0`, clickable: true },
                { text: artistName, url: artistId ? `${protocol}//${host}/${protocol === 'https:' ? 'web/' : ''}#/details?id=${artistId}&serverId=${ApiClient.serverId()}` : null, clickable: !!artistId },
    ...
    async function setupPopoverForElement(element, elementDef, item) {
        if (elementDef.text === 'All Seasons') {
            ...
            if (albums.length === 1) {
                element.addEventListener('click', () => {
                    window.location.href = `${protocol}//${host}/${protocol === 'https:' ? 'web/' : ''}#/details?id=${albums[0].Id}&serverId=${ApiClient.serverId()}`;
                });
            } else {
                element.addEventListener('click', (e) => {
                    e.stopPropagation();
