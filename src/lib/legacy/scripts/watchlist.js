...
(function () {
    ...
    async function initWatchlistSection(section, type) {
        ...
        try {
            ...
            localStorageCache.set(`watchlist_${section}`, optimizedData, userId, WATCHLIST_CACHE_TTL);
            ...
            if (isAdded) {
                ...
                if (item) {
                    ...
                    if (existingIndex === -1) {
                        ...
                        localStorageCache.set(`watchlist_${sectionName}`, optimizedData, userId, WATCHLIST_CACHE_TTL);
                    }
                }
            } else {
    ...
    async function removeItemFromWatchlist(itemId, itemType) {
        ...
        await apiClient.updateUserItemRating(userId, itemId, 'false');
        ...
        try {
            ...
            if (episode.SeasonId) {
                ...
                if (seasonInWatchlist) {
                    const seasonItem = await apiClient.getItem(userId, episode.SeasonId);
            ...
            if (episode.SeriesId) {
                ...
                if (seriesInWatchlist) {
                    const seriesItem = await apiClient.getItem(userId, episode.SeriesId);
    ...
    function createMovieCard(movie) {
        ...
        const imageUrl = movie.ImageTags.Thumb && movie.ImageTags.Thumb ?
            `${serverUrl}/Items/${movie.Id}/Images/Thumb?maxHeight=400&maxWidth=400&tag=${movie.ImageTags.Thumb}` :
            movie.BackdropImageTags && movie.BackdropImageTags[0] ?
                `${serverUrl}/Items/${movie.Id}/Images/Backdrop?maxHeight=400&maxWidth=400&tag=${movie.BackdropImageTags[0]}` :
                movie.ImageTags.Primary ?
                    `${serverUrl}/Items/${movie.Id}/Images/Primary?maxHeight=300&maxWidth=200&tag=${movie.ImageTags.Primary}` :
                    '';
        ...
        card.innerHTML = `
			<a href="${serverUrl}/web/#/details?id=${movie.Id}&serverId=${apiClient.serverId()}">
    ...
    async function createProgressCard(progress) {
        ...
        const imageUrl = window.innerWidth > 768 ?
            series.ImageTags && series.ImageTags.Primary ?
                `${serverUrl}/Items/${series.Id}/Images/Primary?maxHeight=200&maxWidth=140&tag=${series.ImageTags.Primary}` :
                '' :
            series.ImageTags && series.ImageTags.Banner ?
                `${serverUrl}/Items/${series.Id}/Images/Banner?maxWidth=300&height=60&tag=${series.ImageTags.Banner}` :
                '';
        ...
        card.innerHTML = `
			<div class="progress-card-content">
				<div class="progress-poster">
					<a href="${serverUrl}/web/#/details?id=${series.Id}&serverId=${apiClient.serverId()}" class="poster-link">
				...
				<div class="progress-details">
					<div class="progress-header">
						<h3 class="progress-title">
							<a href="${serverUrl}/web/#/details?id=${series.Id}&serverId=${apiClient.serverId()}" class="title-link">
								${series.Name}
							</a>
        ...
        try {
            ...
            const episodesHtml = sortedEpisodes.map(episode => {
                ...
                const episodeImageHtml = hasImage ?
                    `<img src="${serverUrl}/Items/${episode.Id}/Images/Primary?maxHeight=60&maxWidth=100&tag=${episode.ImageTags.Primary}" alt="${episodeTitle}" loading="lazy">` :
					...
					<div class="unwatched-episode-item" data-episode-id="${episode.Id}" data-series-id="${seriesId}" data-season="${episode.ParentIndexNumber}" data-episode="${episode.IndexNumber}" data-name="${episodeTitle}">
						...
						<div class="episode-details">
							<div class="episode-title">
								<a href="${serverUrl}/web/#/details?id=${episode.Id}&serverId=${apiClient.serverId()}" class="episode-link">
    ...
    async function getWatchlistTabIndex() {
        const apiClient = window.ApiClient;
        let serverUrl; let token;

        if (apiClient) {
            serverUrl = apiClient?._serverAddress;
        ...
        try {
            const response = await fetch(`${serverUrl}/CustomTabs/Config`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Emby-Token': token
    ...
    function addCustomMenuLink() {
        ...
        if (window.KefinTweaksUtils && window.KefinTweaksUtils.addCustomMenuLink) {
            fetch(`${serverUrl}/CustomTabs/Config`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Emby-Token': token
    ...
    function addWatchlistButton(overlayContainer) {
        ...
        watchlistButton.addEventListener('click', async (e) => {
            ...
            await apiClient.updateUserItemRating(userId, itemId, newRating);
