...
(function () {
	...
	function optimizeProgressDataForStorage(progressDataArray) {
		const len = progressDataArray.length;
		const result = new Array(len);
		for (let i = 0; i < len; i++) {
			const progress = progressDataArray[i];
			result[i] = {
				// Series info (minimal)
				series: {
	...
	function optimizeMovieDataForStorage(movieDataArray) {
		const len = movieDataArray.length;
		const result = new Array(len);
		for (let i = 0; i < len; i++) {
	...
	function optimizeWatchlistDataForStorage(watchlistDataArray) {
		const len = watchlistDataArray.length;
		const result = new Array(len);
		for (let i = 0; i < len; i++) {
			const item = watchlistDataArray[i];
			const optimizedData = {
				Id: item.Id,
				Type: item.Type,
