...
(function() {
    ...
    function shouldCallHandler(config, view) {
        const pages = config.options.pages;
        ...
        for (let i = 0, len = pages.length; i < len; i++) {
            ...
            if (typeof page === 'string') {
                // Pre-calculate search strings if this becomes a bottleneck, 
                // but for now simple includes is fast.
                if (currentUrl.includes(page)) return true;
            } else if (page instanceof RegExp) {
                if (page.test(currentUrl)) return true;
            }
        }
        return false;
    }
